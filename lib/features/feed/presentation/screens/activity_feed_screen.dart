...
class _ActivityFeedScreenState extends State<ActivityFeedScreen> with SingleTickerProviderStateMixin {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _activityError = 'Unable to load workout activities. Tap to retry.';
        });
      }
    }
  }

  /// Pulls live territory capture events.
  Future<void> _loadEvents() async {
    ...
    try {
      await state.fetchEvents(force: true);
      if (mounted) {
      ...
      if (mounted) {
        setState(() {
          ...
          _eventsError = 'Unable to load capture events. Tap to retry.';
        });
      }
    }
  }
        ...
        children: [
          ...
          RefreshIndicator(
            ...
            child: _loadingEvents
                ...
                : _eventsError != null && state.recentEvents.isEmpty
                    ? Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(_eventsError!, style: TextStyle(color: Colors.grey.shade600, fontSize: 13)),
          ...
          RefreshIndicator(
            ...
            child: _loadingMyActivities
                ? const Center(child: CircularProgressIndicator(color: Color(0xFFE040FB)))
                : _activityError != null && state.activities.isEmpty
                    ? Center(
                        child: Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            ...
                            const SizedBox(height: 12),
                            Text(_activityError!, style: TextStyle(color: Colors.grey.shade600, fontSize: 13)),
                            const SizedBox(height: 16),
                            ElevatedButton.icon(
                              ...
                              Expanded(
                                child: ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    ...
                                    final isClosed = act['closedLoop'] == true || act['isClosedLoop'] == true;
                                    final status = (act['status'] ?? 'COMPLETED').toString();
                                    final distance = (act['distance'] as num?)?.toDouble() ?? 0.0;
                                    final duration = (act['duration'] as num?)?.toInt() ?? 0;
                                    final type = (act['type'] ?? 'RUN').toString();
                                                ...
                                                children: [
                                                  Row(
                                                    ...
                                                    children: [
                                                      ...
                                                      Text(
                                                        _formatDateTime(act['startTime']?.toString()),
                                                  ...
                                                  Text(
                                                    'Distance: ${distance >= 1000 ? '${(distance / 1000).toStringAsFixed(2)} km' : '${distance.toStringAsFixed(0)} m'}  •  Duration: ${_formatDuration(duration)}',
  ...
  Widget _buildWeeklyChart(AppState state) {
    ...
    try {
      ...
      for (var act in state.activities) {
        ...
        final startTimeStr = act['startTime']?.toString();
        ...
        if (startTimeStr != null && startTimeStr.isNotEmpty) {
          try {
            ...
            if (dailyDistance.containsKey(dateKey)) {
              final rawDist = (act['distance'] as num?)?.toDouble() ?? 0.0;
              dailyDistance[dateKey] = dailyDistance[dateKey]! + rawDist;
            }
          } catch (_) {}
        }
      }
                    ...
                    Container(
                      ...
                      child: Stack(
                        ...
                        children: [
                          Container(
                            width: 12,
                            height: barHeight > 0 ? barHeight : 3.0,
                            decoration: BoxDecoration(
                              gradient: barHeight > 0
                                  ...
                                  : null,
                              color: barHeight == 0 ? Colors.grey.shade300 : null,
                              borderRadius: BorderRadius.circular(6),
                            ),
                          ),
                        ],
      ...
      debugPrint('Error building weekly chart: $e');
  ...
  void _showInviteFriendsDialog(BuildContext context, AppState state, dynamic activityId) {
    if (activityId == null) return;
    final int actId = (activityId as num).toInt();

    showDialog(
      context: context,
      builder: (ctx) {
        return AlertDialog(
          title: const Text('Challenge Friend', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
          content: state.friends.isEmpty
              ? const Text('You have no friends added yet. Add friends from the Social tab to challenge them to your ghost routes!')
              : SizedBox(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (c, idx) {
                      final friend = state.friends[idx];
                      ...
                      return ListTile(
                        ...
                        trailing: ElevatedButton(
                          ...
                          onPressed: () async {
                            Navigator.pop(ctx);
                            try {
                              await state.createRouteInvitation(friendId, actId);
                              if (context.mounted) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(content: Text('Challenge sent to $username!')),
                                );
                              }
                            } catch (e) {
                              if (context.mounted) {
