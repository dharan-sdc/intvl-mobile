...
///
/// [How] Employs a multi-tier write-ahead logging (WAL) mechanism to local disk,
/// manages network-interrupted offline submissions, and provides UI responsiveness.
class AppState extends ChangeNotifier with WidgetsBindingObserver {
  ...
  String? currentClientActivityId;
  Duration totalPausedDuration = Duration.zero;

  AppState() {
    WidgetsBinding.instance.addObserver(this);
    _initGpsServiceStatusListener();
    _setupAuthInterceptors();
    _initPersistence();
  }

  /// Configures automated 401 token refresh and session expiration handling.
  void _setupAuthInterceptors() {
    api.onTokenRefreshNeeded = () async {
  ...
  /// Listens to real-time OS location hardware state (GPS toggled on/off in device settings).
  void _initGpsServiceStatusListener() {
    try {
      ...
      _serviceStatusSubscription = Geolocator.getServiceStatusStream().listen((ServiceStatus status) {
        ...
        if (isGpsEnabled != enabled) {
          ...
          if (!enabled) {
            hasGpsSignal = false;
            debugPrint('Device GPS hardware was toggled OFF by user.');
          } else {
            debugPrint('Device GPS hardware was toggled ON by user. Re-syncing stream...');
            _syncLocationStream();
          }
          notifyListeners();
        }
      ...
      debugPrint('Error attaching GPS ServiceStatus stream: $e');
    }
  }

  /// Starts a periodic watchdog to detect GPS signal loss (e.g. tunnels, dense indoors).
  void _startGpsWatchdog() {
    _gpsWatchdogTimer?.cancel();
    _gpsWatchdogTimer = Timer.periodic(const Duration(seconds: 4), (timer) {
      if (isRecording && _lastGpsFixTime != null) {
        final secondsSinceLastFix = DateTime.now().difference(_lastGpsFixTime!).inSeconds;
        final hadSignal = hasGpsSignal;
        hasGpsSignal = secondsSinceLastFix < 8;
        if (hadSignal != hasGpsSignal) {
          notifyListeners();
        }
      }
    });
  ...
  /// Stops the GPS signal watchdog timer.
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      if (isRecording) {
        // If workout stream is already active and healthy, retain it without stream re-creation.
        if (trackingMode != TrackingMode.activeWorkout && trackingMode != TrackingMode.activeWorkoutBackground) {
          ...
          trackingMode = TrackingMode.activeWorkout;
          notifyListeners();
        }
      } else {
      ...
      if (isRecording) {
        // Maintain continuous background location recording with active Foreground Service.
        // Do not tear down or re-initialize the GPS stream to prevent satellite lock loss.
        trackingMode = TrackingMode.activeWorkoutBackground;
        notifyListeners();
      } else {
  ...
  Future<void> _initPersistence() async {
    try {
      // 1. Instantly restore cached user session & local preferences from disk (<20ms)
      ...
      debugPrint('Error during local persistence initialization: $e');
    } finally {
      isInitialized = true;
      notifyListeners();
    }

    // 2. Perform backend health check & offline activity sync asynchronously in the background
    checkBackendConnection().then((online) {
      if (online) {
        fetchTerritories(force: true);
        if (isLoggedIn) {
          fetchActivities(force: true);
          fetchPlayerStats();
          fetchMissions();
          fetchAchievements();
          if (pendingOfflineActivities.isNotEmpty) {
            syncPendingOfflineActivities();
          }
        }
      }
    }).catchError((e) {
      debugPrint('Background backend check failed: $e');
    });
  }
  ...
  List<FriendModel> get friends => friendsList;
  List<FriendshipRequestModel> pendingFriendRequests = [];
  List<SentFriendshipRequestModel> sentFriendRequests = [];
  ...
  List<MissionModel> newlyCompletedMissions = [];
  List<AchievementModel> newlyUnlockedAchievements = [];
  Set<int> _knownCompletedMissionIds = {};
  Set<int> _knownUnlockedAchievementIds = {};
  bool _missionsInitialized = false;
  bool _achievementsInitialized = false;

  /// Safely consumes and clears newly unlocked star badge achievements for celebration dialogs.
  List<AchievementModel> consumeNewlyUnlockedAchievements() {
    final list = List<AchievementModel>.from(newlyUnlockedAchievements);
    newlyUnlockedAchievements.clear();
    return list;
  }

  /// Safely consumes and clears newly completed missions for celebration dialogs.
  List<MissionModel> consumeNewlyCompletedMissions() {
    final list = List<MissionModel>.from(newlyCompletedMissions);
    newlyCompletedMissions.clear();
    return list;
  }
  ...
  Future<void> updateTrackingMode(TrackingMode newMode) async {
    ...
    // Retain uninterrupted active GPS stream between foreground workout and background workout
    if ((trackingMode == TrackingMode.activeWorkout && newMode == TrackingMode.activeWorkoutBackground) ||
        (trackingMode == TrackingMode.activeWorkoutBackground && newMode == TrackingMode.activeWorkout)) {
  ...
  // User Auth & Offline Snapshot Persistence Helpers
  ...
  /// Persists the latest synchronized player state and model snapshots to local disk.
  ///
  /// [Why] Enables instant offline UI boot without flashing blank or level 1 defaults.
  Future<void> _savePlayerSnapshotToDisk() async {
    if (userId == null) return;
    try {
      final prefs = await SharedPreferences.getInstance();
      final uid = userId!;
      await prefs.setInt('cached_level_$uid', level);
      await prefs.setInt('cached_xp_$uid', xp);
      await prefs.setInt('cached_coins_$uid', coins);
      await prefs.setInt('cached_streak_$uid', streak);
      await prefs.setInt('cached_contribution_score_$uid', contributionScore);
      await prefs.setDouble('cached_total_distance_$uid', totalDistance);
      await prefs.setInt('cached_total_activities_$uid', totalActivities);
      await prefs.setDouble('cached_total_territory_area_$uid', totalTerritoryArea);
      await prefs.setInt('cached_territories_captured_$uid', territoriesCaptured);
      await prefs.setInt('cached_territories_lost_$uid', territoriesLost);
      await prefs.setInt('cached_territories_defended_$uid', territoriesDefended);
      await prefs.setInt('cached_xp_req_current_$uid', xpRequiredForCurrentLevel);
      await prefs.setInt('cached_xp_req_next_$uid', xpRequiredForNextLevel);
      await prefs.setDouble('cached_weight_$uid', weight);
      await prefs.setDouble('cached_height_$uid', height);
      await prefs.setInt('cached_age_$uid', age);
      await prefs.setString('cached_gender_$uid', gender);

      if (territories.isNotEmpty) {
        await prefs.setString('cached_territories_$uid', jsonEncode(territories.map((t) => t.toJson()).toList()));
      }
      if (dailyMissions.isNotEmpty) {
        await prefs.setString('cached_daily_missions_$uid', jsonEncode(dailyMissions.map((m) => m.toJson()).toList()));
      }
      if (weeklyMissions.isNotEmpty) {
        await prefs.setString('cached_weekly_missions_$uid', jsonEncode(weeklyMissions.map((m) => m.toJson()).toList()));
      }
      if (achievements.isNotEmpty) {
        await prefs.setString('cached_achievements_$uid', jsonEncode(achievements.map((a) => a.toJson()).toList()));
      }
      if (myCampaigns.isNotEmpty) {
        await prefs.setString('cached_my_campaigns_$uid', jsonEncode(myCampaigns.map((c) => c.toJson()).toList()));
      }
      if (leaderboard.isNotEmpty) {
        await prefs.setString('cached_leaderboard_$uid', jsonEncode(leaderboard.map((l) => l.toJson()).toList()));
      }
      if (friendsList.isNotEmpty) {
        await prefs.setString('cached_friends_$uid', jsonEncode(friendsList.map((f) => f.toJson()).toList()));
      }
    } catch (e) {
      debugPrint('Error saving player snapshot to disk: $e');
    }
  }

  /// Wipes all user-specific credentials and snapshot cache keys from local storage.
  Future<void> _clearUserSession() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final uid = userId;
      if (uid != null) {
        await prefs.remove('cached_level_$uid');
        await prefs.remove('cached_xp_$uid');
        await prefs.remove('cached_coins_$uid');
        await prefs.remove('cached_streak_$uid');
        await prefs.remove('cached_contribution_score_$uid');
        await prefs.remove('cached_total_distance_$uid');
        await prefs.remove('cached_total_activities_$uid');
        await prefs.remove('cached_total_territory_area_$uid');
        await prefs.remove('cached_territories_captured_$uid');
        await prefs.remove('cached_territories_lost_$uid');
        await prefs.remove('cached_territories_defended_$uid');
        await prefs.remove('cached_xp_req_current_$uid');
        await prefs.remove('cached_xp_req_next_$uid');
        await prefs.remove('cached_weight_$uid');
        await prefs.remove('cached_height_$uid');
        await prefs.remove('cached_age_$uid');
        await prefs.remove('cached_gender_$uid');
        await prefs.remove('cached_territories_$uid');
        await prefs.remove('cached_daily_missions_$uid');
        await prefs.remove('cached_weekly_missions_$uid');
        await prefs.remove('cached_achievements_$uid');
        await prefs.remove('cached_my_campaigns_$uid');
        await prefs.remove('cached_leaderboard_$uid');
        await prefs.remove('cached_friends_$uid');
      }
  ...
  /// Restores cached login session and offline player snapshot instantly (<15ms).
  Future<void> _restoreUserSession() async {
    try {
      ...
      if (loggedIn && savedId != null) {
        ...
        // Restore cached offline player snapshot instantly (<15ms)
        level = prefs.getInt('cached_level_$savedId') ?? level;
        xp = prefs.getInt('cached_xp_$savedId') ?? xp;
        coins = prefs.getInt('cached_coins_$savedId') ?? coins;
        streak = prefs.getInt('cached_streak_$savedId') ?? streak;
        contributionScore = prefs.getInt('cached_contribution_score_$savedId') ?? contributionScore;
        totalDistance = prefs.getDouble('cached_total_distance_$savedId') ?? totalDistance;
        totalActivities = prefs.getInt('cached_total_activities_$savedId') ?? totalActivities;
        totalTerritoryArea = prefs.getDouble('cached_total_territory_area_$savedId') ?? totalTerritoryArea;
        territoriesCaptured = prefs.getInt('cached_territories_captured_$savedId') ?? territoriesCaptured;
        territoriesLost = prefs.getInt('cached_territories_lost_$savedId') ?? territoriesLost;
        territoriesDefended = prefs.getInt('cached_territories_defended_$savedId') ?? territoriesDefended;
        xpRequiredForCurrentLevel = prefs.getInt('cached_xp_req_current_$savedId') ?? xpRequiredForCurrentLevel;
        xpRequiredForNextLevel = prefs.getInt('cached_xp_req_next_$savedId') ?? xpRequiredForNextLevel;
        weight = prefs.getDouble('cached_weight_$savedId') ?? weight;
        height = prefs.getDouble('cached_height_$savedId') ?? height;
        age = prefs.getInt('cached_age_$savedId') ?? age;
        gender = prefs.getString('cached_gender_$savedId') ?? gender;

        // Restore cached model lists
        final territoriesRaw = prefs.getString('cached_territories_$savedId');
        if (territoriesRaw != null && territoriesRaw.isNotEmpty) {
          try {
            final List<dynamic> list = jsonDecode(territoriesRaw);
            territories = list.map((j) => TerritoryModel.fromJson(j)).toList();
          } catch (_) {}
        }

        final dailyMissionsRaw = prefs.getString('cached_daily_missions_$savedId');
        if (dailyMissionsRaw != null && dailyMissionsRaw.isNotEmpty) {
          try {
            final List<dynamic> list = jsonDecode(dailyMissionsRaw);
            dailyMissions = list.map((j) => MissionModel.fromJson(j)).toList();
          } catch (_) {}
        }

        final weeklyMissionsRaw = prefs.getString('cached_weekly_missions_$savedId');
        if (weeklyMissionsRaw != null && weeklyMissionsRaw.isNotEmpty) {
          try {
            final List<dynamic> list = jsonDecode(weeklyMissionsRaw);
            weeklyMissions = list.map((j) => MissionModel.fromJson(j)).toList();
          } catch (_) {}
        }

        final achievementsRaw = prefs.getString('cached_achievements_$savedId');
        if (achievementsRaw != null && achievementsRaw.isNotEmpty) {
          try {
            final List<dynamic> list = jsonDecode(achievementsRaw);
            achievements = list.map((j) => AchievementModel.fromJson(j)).toList();
          } catch (_) {}
        }

        final myCampaignsRaw = prefs.getString('cached_my_campaigns_$savedId');
        if (myCampaignsRaw != null && myCampaignsRaw.isNotEmpty) {
          try {
            final List<dynamic> list = jsonDecode(myCampaignsRaw);
            myCampaigns = list.map((j) => CampaignModel.fromJson(j)).toList();
          } catch (_) {}
        }

        final leaderboardRaw = prefs.getString('cached_leaderboard_$savedId');
        if (leaderboardRaw != null && leaderboardRaw.isNotEmpty) {
          try {
            final List<dynamic> list = jsonDecode(leaderboardRaw);
            leaderboard = list.map((j) => LeaderboardEntryModel.fromJson(j)).toList();
          } catch (_) {}
        }

        final friendsRaw = prefs.getString('cached_friends_$savedId');
        if (friendsRaw != null && friendsRaw.isNotEmpty) {
          try {
            final List<dynamic> list = jsonDecode(friendsRaw);
            friendsList = list.map((j) => FriendModel.fromJson(j)).toList();
          } catch (_) {}
        }

        // Initialize known completed IDs from the restored snapshot
        final restoredCompleted = [...dailyMissions, ...weeklyMissions]
            .where((m) => m.status == 'COMPLETED' || m.status == 'CLAIMED')
            .map((m) => m.id);
        _knownCompletedMissionIds = restoredCompleted.toSet();
        _missionsInitialized = dailyMissions.isNotEmpty || weeklyMissions.isNotEmpty;

        final restoredUnlocked = achievements.where((a) => a.isUnlocked).map((a) => a.id);
        _knownUnlockedAchievementIds = restoredUnlocked.toSet();
        _achievementsInitialized = achievements.isNotEmpty;

        await _loadLocationPreferences();
        notifyListeners();
        
        // Stale-While-Revalidate: revalidate fresh data in background
        fetchTerritories(force: true);
        fetchActivities(force: true);
        fetchPlayerStats();
        fetchMissions();
  ...
  /// Seamlessly extends the user session by exchanging the refresh token for a new access token.
  Future<bool> refreshSession() async {
    if (refreshToken == null || refreshToken!.isEmpty) {
      debugPrint('⚠️ [AUTH] Cannot refresh session: No refresh token stored.');
      return false;
    }
    try {
      debugPrint('🔄 [AUTH] Attempting silent token refresh...');
      final res = await api.refreshToken(refreshToken!);
      ...
      await _saveUserSession();
      debugPrint('✅ [AUTH] Token refreshed successfully. New access token active.');
      notifyListeners();
      return true;
    } catch (e) {
  ...
  void logout({bool sessionExpired = false}) {
    ...
    rewards.clear();
    myCampaigns.clear();
    discoverCampaigns.clear();
    completedCampaigns.clear();
    newlyCompletedMissions.clear();
    newlyUnlockedAchievements.clear();
    _knownCompletedMissionIds.clear();
    _knownUnlockedAchievementIds.clear();
    _missionsInitialized = false;
  ...
  Future<void> _saveRecordingSessionToDisk({bool force = false}) async {
    ...
    try {
      ...
      final sessionData = {
        ...
        'clientActivityId': currentClientActivityId,
  ...
  Future<void> restoreRecordingSession() async {
    try {
      ...
      if (sessionData['isRecording'] == true) {
        currentClientActivityId = sessionData['clientActivityId'];
    ...
    for (var act in List<Map<String, dynamic>>.from(pendingOfflineActivities)) {
      try {
        ...
        await api.submitActivity(
          ...
          points: pointsRaw,
          clientActivityId: act['clientActivityId'] ?? act['id'],
  ...
  void startRecording() {
    ...
    currentClientActivityId = 'act_${DateTime.now().microsecondsSinceEpoch}_${100000 + Random().nextInt(900000)}';
    pathPoints.clear();
    durationSeconds = 0;
    distanceMeters = 0.0;
    sessionStartTime = DateTime.now();
    _lastRecordedPointTime = DateTime.now();
  ...
  Future<void> _syncLocationStream() async {
    ...
    _positionStreamSubscription = Geolocator.getPositionStream(locationSettings: locationSettings)
      ...
      if (isRecording && !isSimulationMode) {
        ...
        // 1. Noise gate: ignore low accuracy GPS spikes (tightened from 25.0m to 15.0m)
        if (acc <= 15.0) {
          if (pathPoints.isNotEmpty) {
            final lastPoint = pathPoints.last;
            final double stepDistance = const Distance().as(
              LengthUnit.Meter,
              lastPoint,
              newPoint,
            );

            // 2. Velocity outlier gate: compute time delta to reject impossible teleport jumps (> 15 m/s ≈ 54 km/h)
            final now = DateTime.now();
            final double elapsedSec = _lastRecordedPointTime != null
                ? (now.difference(_lastRecordedPointTime!).inMilliseconds / 1000.0)
                : 1.0;

            final double speedMps = elapsedSec > 0.1 ? (stepDistance / elapsedSec) : 0.0;
            final bool isPhysicallyPossible = speedMps <= 15.0;

            // 3. Stationary jitter suppression: ignore < 2.0m jitter unless moving with reported speed >= 0.5 m/s
            if (isPhysicallyPossible) {
              ...
              debugPrint('Rejected GPS outlier teleport: ${stepDistance.toStringAsFixed(1)}m in ${elapsedSec.toStringAsFixed(1)}s (${speedMps.toStringAsFixed(1)} m/s, accuracy: ${acc}m)');
            }
          } else {
            pathPoints.add(newPoint);
            _lastRecordedPointTime = DateTime.now();
  ...
  Future<Map<String, dynamic>?> completeAndSubmitActivity() async {
    ...
    final savedClientActivityId = currentClientActivityId ?? 
        'act_${DateTime.now().microsecondsSinceEpoch}_${100000 + Random().nextInt(900000)}';

    Map<String, dynamic> res;
    try {
      res = await api.submitActivity(
        userId: savedUserId,
        type: savedType,
        duration: savedDuration,
        distance: savedDistance,
        points: apiPoints,
        clientActivityId: savedClientActivityId,
      ...
      debugPrint('Primary activity submission failed (offline / network error / timeout): $e');
      errorMessage = ErrorHandler.parse(e, 'Failed to submit workout activity.');

      // Resilient Fallback: Queue activity for offline synchronization with preserved clientActivityId
    ...
    // State Finalization: Unlock UI instantly (< 50ms)
    isRecording = false;
    pathPoints.clear();
    sessionStartTime = null;
    _lastRecordedPointTime = null;
    ...
    // Synchronous refresh to evaluate newly completed missions & star badge achievements
    await _refreshAppDataPostWorkout(savedDistance, res);

    return res;
  }

  /// Refreshes all global and user data after a workout and detects newly achieved goals.
  Future<void> _refreshAppDataPostWorkout(double savedDistance, Map<String, dynamic> res) async {
    try {
      final priorCompletedMissionIds = {
        ...dailyMissions.where((m) => m.status == 'COMPLETED' || m.status == 'CLAIMED').map((m) => m.id),
        ...weeklyMissions.where((m) => m.status == 'COMPLETED' || m.status == 'CLAIMED').map((m) => m.id),
      };
      final priorUnlockedAchievementIds = achievements.where((a) => a.isUnlocked).map((a) => a.id).toSet();

      await Future.wait([
        fetchTerritories(force: true),
        fetchActivities(force: true),
        fetchLeaderboard(force: true),
        fetchPlayerStats(force: true),
        fetchProgression(force: true),
        fetchAchievements(force: true),
        fetchMissions(force: true),
        fetchFriends(force: true),
        fetchPendingFriendRequests(force: true),
        fetchSentFriendRequests(force: true),
        fetchRouteInvitations(force: true),
        fetchMyCampaigns(force: true),
      ]).catchError((e) {
        debugPrint('Non-critical secondary fetch error during post-workout sync: $e');
        return <void>[];
      });

      await _evaluateCampaignImpactsForRun(savedDistance);

      newlyCompletedMissions = [...dailyMissions, ...weeklyMissions]
          .where((m) => (m.status == 'COMPLETED' || m.status == 'CLAIMED') && !priorCompletedMissionIds.contains(m.id))
          .toList();

      newlyUnlockedAchievements = achievements
          .where((a) => a.isUnlocked && !priorUnlockedAchievementIds.contains(a.id))
          .toList();
      ...
      debugPrint('Silent error in post-workout sync: $e');
    }
  }
  ...
  /// Queries all global territories for rendering H3 polygons with 1min cache TTL.
  Future<void> fetchTerritories({bool force = false}) async {
    ...
    try {
      ...
      territories = data.map<TerritoryModel>((json) => TerritoryModel.fromJson(json)).toList();
      _savePlayerSnapshotToDisk();
  ...
  Future<void> fetchMissions({bool force = false}) async {
    ...
    try {
      ...
      final allLoaded = [...dailyMissions, ...weeklyMissions];
      final currentCompleted = allLoaded
          .where((m) => m.status == 'COMPLETED' || m.status == 'CLAIMED')
          .toList();

      if (!_missionsInitialized) {
        _knownCompletedMissionIds = currentCompleted.map((m) => m.id).toSet();
        _missionsInitialized = true;
      } else {
        final newCompletions = currentCompleted
            .where((m) => !_knownCompletedMissionIds.contains(m.id))
            .toList();
        if (newCompletions.isNotEmpty) {
          for (final m in newCompletions) {
            _knownCompletedMissionIds.add(m.id);
          }
          final existingIds = newlyCompletedMissions.map((m) => m.id).toSet();
          for (final m in newCompletions) {
            if (!existingIds.contains(m.id)) {
              newlyCompletedMissions.add(m);
            }
          }
        }
      }
  ...
  Future<void> fetchAchievements({bool force = false}) async {
    ...
    try {
      ...
      final currentUnlocked = achievements.where((a) => a.isUnlocked).toList();
      if (!_achievementsInitialized) {
        _knownUnlockedAchievementIds = currentUnlocked.map((a) => a.id).toSet();
        ...
        if (newUnlocks.isNotEmpty) {
          for (final a in newUnlocks) {
            _knownUnlockedAchievementIds.add(a.id);
          }
          final existingIds = newlyUnlockedAchievements.map((a) => a.id).toSet();
          for (final a in newUnlocks) {
            if (!existingIds.contains(a.id)) {
              newlyUnlockedAchievements.add(a);
            }
          }
        }
      }
  ...
  Future<void> fetchPendingFriendRequests({bool force = false}) async {
    ...
    try {
      ...
      pendingFriendRequests = list.map((json) => FriendshipRequestModel(
        ...
        senderUsername: json['senderUsername'] ?? 'Player',
        senderColor: json['senderColor'] ?? '#888888',
        createdAt: json['createdAt'] ?? '',
      )).toList();
      _recordFetchEnd(cacheKey);
      notifyListeners();
  ...
  /// Loads outgoing pending friend requests sent by the user with 45s cache TTL.
  Future<void> fetchSentFriendRequests({bool force = false}) async {
    if (userId == null) return;
    const cacheKey = 'sent_friend_requests';
    if (!_shouldFetch(cacheKey, ttl: const Duration(seconds: 45), force: force)) return;
    _recordFetchStart(cacheKey);
    try {
      final list = await api.getSentFriendRequests(userId!);
      sentFriendRequests = list.map((json) => SentFriendshipRequestModel(
        id: json['id'],
        receiverId: json['receiverId'] ?? 0,
        receiverUsername: json['receiverUsername'] ?? 'Player',
        receiverColor: json['receiverColor'] ?? '#888888',
        createdAt: json['createdAt'] ?? '',
      )).toList();
      _recordFetchEnd(cacheKey);
      notifyListeners();
      ...
      _isFetching[cacheKey] = false;
      debugPrint('Error loading sent friend requests: $e');
    }
  }
  ...
  Future<bool> sendFriendRequest(String username) async {
    ...
    try {
      ...
      await fetchSentFriendRequests(force: true);
      await fetchPendingFriendRequests(force: true);
  ...
  Future<bool> cancelSentFriendRequest(int friendshipId) async {
    if (userId == null) return false;
    _setLoading(true);
    errorMessage = null;
    try {
      ...
      errorMessage = ErrorHandler.parse(e, 'Failed to cancel sent friend request.');
      _setLoading(false);
      return false;
    }
  }
...
class TerritoryModel {
  ...
  Map<String, dynamic> toJson() => {
    ...
    'status': status,
    'paths': paths.map((p) => p.map((pt) => {'lat': pt.latitude, 'lng': pt.longitude}).toList()).toList(),
  };
  ...
  factory TerritoryModel.fromJson(Map<String, dynamic> json) {
    ...
    if (json['paths'] != null) {
      for (var p in json['paths']) {
        List<LatLng> pts = [];
        for (var pt in p) {
          pts.add(LatLng((pt['lat'] as num).toDouble(), (pt['lng'] as num).toDouble()));
        }
        paths.add(pts);
      }
    }
...
class MissionModel {
  ...
  factory MissionModel.fromJson(Map<String, dynamic> json) => MissionModel(
    ...
    contributionReward: json['contributionReward'] ?? 0,
    date: json['date'] ?? '',
  );
}
...
class RewardModel {
  ...
  factory RewardModel.fromJson(Map<String, dynamic> json) => RewardModel(
    ...
    description: json['description'] ?? '',
    type: json['type'] ?? '',
    unlockedAtLevel: json['unlockedAtLevel'] ?? 1,
    costInCoins: json['costInCoins'] ?? 0,
    isClaimed: json['isClaimed'] ?? false,
  );
}
...
class FriendModel {
  ...
  factory FriendModel.fromJson(Map<String, dynamic> json) => FriendModel(
    ...
    username: json['username'] ?? '',
    color: json['color'] ?? '#888888',
    totalTerritoryArea: (json['totalTerritoryArea'] as num?)?.toDouble() ?? 0.0,
  );
}
...
class SentFriendshipRequestModel {
  final int id;
...
class CampaignModel {
  ...
  Map<String, dynamic> toJson() => {
    ...
    'individualTargetKm': individualTargetKm,
    'status': status,
    'isJoined': isJoined,
